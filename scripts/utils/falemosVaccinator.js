...
for (let user of game.users.keys()) {
  ...
  jugadores.push(game.users.get(user).name);
}
const numeroJugadores = jugadores.length;
const nombresJugadores = jugadores.join();
const numeracionJugadores = Array(jugadores.length)
  .fill()
  .map((element, index) => index + 1)
  .join();

let roles = [];
for (let user of game.users.keys()) roles.push(game.users.get(user).role);
...
function falemosCalculator(
  ...
  iordenfix.forEach(function (item, i) {
    if (item < 1) iordenfix[i] = iordenmax;
  });
  ...
  for (var i = 0; i < iordenfix.length; ++i) iordenfixindices[i] = i;
  ...
  if (iusers.length > inamesfix.length)
    inamesfix = inamesfix.concat(
      Array(iusers.length - inamesfix.length).fill("")
    );
  ...
  for (var i = 0; i < Math.min(resultadoLibres.length, iusers.length); ++i) {
    resultadofinal[iordenfixindices[i]] = resultadoLibres[i].concat(
      iusers[iordenfixindices[i]],
      inamesfix[iordenfixindices[i]]
    );
  }
  ...
  for (let i = 0; i < nuFilas; i++) {
    ...
    for (let j = 0; j < nuColumnas; j++) {
      ...
      if (datostabla[k][2] === true) {
        ...
        tabla = tabla + '<td style="text-align:center;background-color:' + colorCelda + '">' + '${game.i18n.localize(
          "FALEMOS.vaccinator.simulateEmpty"
        )}' + '</td>\\n';
      }
      k++
    }
  ...
  <form>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="tg-d6y8" colspan="3"><input type="number" id="ancho" name="ancho" min=1 value=${
          window.innerWidth
        }></td>
        <td class="tg-d6y8" colspan="2">${game.i18n.localize(
          "FALEMOS.vaccinator.height"
        )}:</td>
        <td class="tg-d6y8" colspan="3"><input type="number" id="alto" name="alto" min=1 value=${
          window.innerHeight
        }></td>
      </tr>
      <tr>
        <td class="tg-d6y8" colspan="6">${game.i18n.localize(
          "FALEMOS.vaccinator.margins"
        )}:</td>
        <td class="tg-d6y8" colspan="1"><input type="number" id="marizq" name="marizq" min=0 value=${
          document.getElementById("controls").clientWidth
        }></td>
        <td class="tg-d6y8" colspan="1"><input type="number" id="mararr" name="mararr" min=0 value=${
          document.getElementById("scene-list") === null
            ? 50
            : document.getElementById("scene-list").clientHeight
        }></td>
        <td class="tg-d6y8" colspan="1"><input type="number" id="marder" name="marder" min=0 value=${
          document.getElementById("sidebar").offsetWidth
        }></td>
        <td class="tg-d6y8" colspan="1"><input type="number" id="maraba" name="maraba" min=0 value=${
      ...
      <tr>
        <td class="tg-049l" colspan="10">${game.i18n.localize(
          "FALEMOS.vaccinator.hideHint"
        )}</td>
      </tr>
      <tr>
        <td class="tg-r5a9" colspan="10"><span style="color:#FFF"><b>${game.i18n.localize(
          "FALEMOS.vaccinator.world"
        )}</b></span></td>
      </tr>
      <tr>
        <td class="tg-d6y8" colspan="1">${game.i18n.localize(
          "FALEMOS.vaccinator.users"
        )}:</td>
        ...
        <td class="tg-d6y8" colspan="1"><input type="number" id="nRows" name="nRows" min=1 value=${
          slotsOptimo.rows
        }></td>
        <td class="tg-d6y8" colspan="1">${game.i18n.localize(
          "FALEMOS.vaccinator.columns"
        )}:</td>
        <td class="tg-d6y8" colspan="1"><input type="number" id="nCols" name="nCols" min=1 value=${
          slotsOptimo.columns
        }></td>
        <td class="tg-d6y8" colspan="1">${game.i18n.localize(
      ...
      <tr>
        ...
        document.getElementById('roles').value)" type="button">${game.i18n.localize(
          "FALEMOS.vaccinator.simulate"
        )}</button></td>
      </tr>
      <tr>
      ...
      "FALEMOS.vaccinator.columnsHint"
    )}; <b>${game.i18n.localize(
      "FALEMOS.vaccinator.emptyslots"
    )}</b>:  ${game.i18n.localize(
      "FALEMOS.vaccinator.emptyslotsHint"
    )}; <b>${game.i18n.localize(
      "FALEMOS.vaccinator.usernames"
    )}</b>: ${game.i18n.localize(
      "FALEMOS.vaccinator.usernamesHint"
    )}; <b>${game.i18n.localize(
      "FALEMOS.vaccinator.roles"
    )}</b>: ${game.i18n.localize(
      "FALEMOS.vaccinator.rolesHint"
    )} (1: ${game.i18n.localize("USER.RolePlayer")}, 2: ${game.i18n.localize(
      "USER.RoleTrusted"
    )}, 3: ${game.i18n.localize("USER.RoleAssistant")}, 4: ${game.i18n.localize(
      "USER.RoleGamemaster"
    )}); <b>${game.i18n.localize(
      "FALEMOS.vaccinator.playerorder"
    )}</b>:  ${game.i18n.localize(
      "FALEMOS.vaccinator.playerorderHint"
    )}; <b>${game.i18n.localize(
      "FALEMOS.vaccinator.simulate"
    )}</b>:  ${game.i18n.localize("FALEMOS.vaccinator.simulateHint")}</td>
      </tr>
      <tr>
        <td class="tg-r5a9" colspan="10"><b>${game.i18n.localize(
          "FALEMOS.vaccinator.layout"
        )}</b></td>
      </tr>
      <tr>
        <td class="tg-d6y8" colspan="2">${game.i18n.localize(
          "FALEMOS.vaccinator.separation"
        )}:</td>
        <td class="tg-d6y8" colspan="2"><input type="number" id="sepmin" name="sepmin" min=0 value=10></td>
        <td class="tg-d6y8" colspan="2">${game.i18n.localize(
          "FALEMOS.vaccinator.overlays"
        )}:</td>
      ...
      <tr>
        ...
        <td class="tg-d6y8" colspan="1">${game.i18n.localize(
          "FALEMOS.CameraNameColor"
        )}:</td>
        <td class="tg-d6y8" colspan="1"><input type="color" id="namecolor" name="namecolor"></td>
      </tr>
			<tr>
        <td class="tg-d6y8" colspan="1">${game.i18n.localize(
          "FALEMOS.vaccinator.frames"
        )}:</td>
        <td class="tg-d6y8" colspan="8"><input type="text" id="marcos" name="marcos" value=""></td>
        <td class="tg-d6y8" colspan="1"><button id="marcosSeleccionar" onclick="selectImageMult()" type="button">${game.i18n.localize(
          "File Path"
        )}</button></td>
      </tr>
      <tr>
      ...
      "FALEMOS.vaccinator.overlaysHint"
    )}; <b>${game.i18n.localize(
      "FALEMOS.vaccinator.names"
    )}</b>: ${game.i18n.localize(
      "FALEMOS.vaccinator.namesHint"
    )}; <b>${game.i18n.localize(
      "FALEMOS.CameraNameFontText"
    )}</b>: ${game.i18n.localize(
      "FALEMOS.CameraNameFontNotes"
    )}; <b>${game.i18n.localize(
      "FALEMOS.CameraNameFontSize"
    )}</b>: ${game.i18n.localize(
      "FALEMOS.vaccinator.CameraNameFontSizeHint"
    )}; <b>${game.i18n.localize(
      "FALEMOS.CameraNameColor"
    )}</b>: ${game.i18n.localize(
      "FALEMOS.vaccinator.CameraNameColorHint"
    )}; <b>${game.i18n.localize(
      "FALEMOS.vaccinator.frames"
    )}</b>: ${game.i18n.localize("FALEMOS.vaccinator.framesHint")}</td>
      </tr>  
      <tr>
        <td class="tg-r5a9" colspan="10"><b>${game.i18n.localize(
          "FALEMOS.vaccinator.othersettings"
        )}</b></td>
      </tr>
      <tr>
        <td class="tg-d6y8" colspan="1">${game.i18n.localize(
          "FALEMOS.CameraGeometryText"
        )}:</td>
        ...
        <td class="tg-d6y8" colspan="2"><select id="efecto" name="efecto">
          ...
          <option value="Blue">${game.i18n.localize(
            "FALEMOS.camera.effects.blue"
          )}</option>
          <option value="Red">${game.i18n.localize(
            "FALEMOS.camera.effects.red"
          )}</option>
          <option value="Edges">${game.i18n.localize(
            "FALEMOS.camera.effects.edges"
          )}</option>
          <option value="Green">${game.i18n.localize(
            "FALEMOS.camera.effects.green"
          )}</option>
        </select></td>
        <td class="tg-d6y8" colspan="2">${game.i18n.localize(
          "FALEMOS.SceneFitText"
        )}:</td>
      ...
      "FALEMOS.CameraGeometryNotes"
    )}; <b>${game.i18n.localize(
      "FALEMOS.CameraEffectText"
    )}</b>: ${game.i18n.localize(
      "FALEMOS.CameraEffectNotes"
    )}; <b>${game.i18n.localize(
      "FALEMOS.SceneFitText"
    )}</b>: ${game.i18n.localize("FALEMOS.SceneFitNotes")}</td>
      </tr>    
    </tbody>
    </table>
  </form>
  `,
    buttons: {
      ...
      macrodisable: {
        ...
        label: `${game.i18n.localize(
          "FALEMOS.vaccinator.ExportDisableToMacro"
        )}`,
        callback: () => {
          applyChanges = false;
          saveCustomMacro = false;
    ...
    close: (html) => {
      ...
      if (saveDisableMacro) {
        ...
        //console.log("---------------------------------");
        //console.log("---------------------------------");
        game.falemos.sceneConfigToMacro(
